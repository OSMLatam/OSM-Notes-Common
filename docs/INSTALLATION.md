...
flowchart TD
    ...
    VERIFY4 -->|OK| VIEWER[5. OSM-Notes-Viewer]
    VERIFY4 -->|Error| TROUBLESHOOT4[Troubleshooting]
    VIEWER --> VERIFY5{Verify Viewer}
    VERIFY5 -->|OK| API[6. OSM-Notes-API]
    VERIFY5 -->|Error| TROUBLESHOOT5[Troubleshooting]
    API --> VERIFY6{Verify API}
...
5. **OSM-Notes-Viewer** - Requires Data
6. **OSM-Notes-API** - Requires Analytics
...
#### 5.2 Install Dependencies (Optional, for development)
...
```bash
# Edit config/api-config.js if needed
# Default uses: https://osm-notes.github.io/OSM-Notes-Data/
```

#### 5.4 Serve Locally (Development)
...
#### 5.5 Deploy to GitHub Pages (Production)
...
#### 6.4 Create Database User (Read-Only)
